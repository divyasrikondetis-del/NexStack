...
const getSecurityLogs = async (req, res) => {
  try {
    const sessions = await LoginSession.find().populate("userId", "name email role").sort({ createdAt: -1 }).lean();
    res.json({ sessions });
  } catch (error) {
...
module.exports = { getDashboard, getReports, reviewReport, getSecurityLogs };
